...
const C_match = require("../models/C_match");

// Controller to add a new match
const addMatch = async (req, res) => {
  try {
    const { name, status } = req.body;
    const newMatch = new C_match({ name, status });
    await newMatch.save();
    res.json(newMatch);
  } catch (error) {
    console.error(error);
...
const getMatches = async (req, res) => {
  try {
    const matches = await C_match.find();
    res.json(matches);
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal Server Error" });
...
const updateMatch = async (req, res) => {
  try {
    ...
    const existingMatch = await C_match.findById(id);
    if (!existingMatch) {
...
const deleteMatch = async (req, res) => {
  try {
    ...
    // Check if the match exists
    const existingMatch = await C_match.findByIdAndDelete(id);
    if (!existingMatch) {
...
const addScoredetails = async (req, res) => {
  try {
    ...
    const match = await C_match.findById(id);
...
const updateScoredetails = async (req, res) => {
  try {
    ...
    const match = await C_match.findOne({ "scores._id": scoreId });
...
const deleteScoredetails = async (req, res) => {
  ...
  try {
    const existingMatch = await C_match.findById(matchId);
    if (!existingMatch) {
      return res.status(404).json({ message: "Match not found" });
    }
...
const addPlayers = async (req, res) => {
  ...
  try {
    const match = await C_match.findById(matchId);
...
const updatePlayerDetails = async (req, res) => {
  try {
    ...
    const existingMatch = await C_match.findOne({ "players._id": playerId });
