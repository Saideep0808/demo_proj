...
const [newMatch, setNewMatch] = useState({ name: "", status: "future" });
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/api/auth/getMatches_crick")
...
const handleInputChange = (e) => {
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateMatch_crick/${selectedMatch._id}`,
          newMatch
        )
        .then((response) => {
          setMatches(
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_crick", newMatch)
        .then((response) => {
  ...
  const handleDeleteMatch = () => {
    if (selectedMatch) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteMatch_crick/${selectedMatch._id}`
        )
        .then(() => {
          setMatches(
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_crick/${match._id}`)
      .then((response) => setScores(response.data))
      .catch((error) => console.error(error));
    axios
      .get(
        `http://localhost:5000/api/auth/getPlayers_crick/${match._id}?team=TeamA`
      )
    ...
    axios
      .get(
        `http://localhost:5000/api/auth/getPlayers_crick/${match._id}?team=TeamB`
      )
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_crick/${selectedMatch._id}`,
          scoreFormData
        )
        .then((response) => {
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_crick/${selectedScoreDetails._id}`,
          scoreFormData
        )
        .then((response) => {
          setScores(
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_crick/${scoreId}`,
          {
  ...
  const handleAddPlayer = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addPlayers_crick/${selectedMatch._id}`,
          playerFormData
        )
        .then((response) => {
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updatePlayerDetails_crick/${_id}`,
          {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deletePlayerDetails_crick/${selectedPlayerDetails._id}`
        )
        .then(() => {
          const updatedPlayers =
