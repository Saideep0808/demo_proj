 ...
 const isAdmin = localStorage.getItem("isAdmin") === "true"; // Retrieve and parse admin status
  ...
  const handleRowClick = (match) => {
    ...
    axios
      ...
      .catch((error) => console.error(error));
    axios
      .get(`http://localhost:5000/api/auth/getPlayers_bad/${match._id}?team=TeamA`)
      .then((response) => setPlayersTeamA(response.data))
      .catch((error) => console.error(error));

    axios
      .get(`http://localhost:5000/api/auth/getPlayers_bad/${match._id}?team=TeamB`)
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_bad/${selectedScoreDetails._id}`,
          {
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
            round2: parseInt(scoreFormData.round2), // Parse round2 to integer
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_bad/${scoreId}`,
          {
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(`http://localhost:5000/api/auth/updatePlayerDetails_bad/${_id}`, {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deletePlayerDetails_bad/${selectedPlayerDetails._id}`
        )
        .then(() => {
          const updatedPlayers =
