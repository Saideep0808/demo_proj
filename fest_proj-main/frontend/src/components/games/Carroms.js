 ...
 const isAdmin = localStorage.getItem("isAdmin") === "true"; // Retrieve and parse admin status
  ...
  const handleAddMatch = () => {
    if (selectedMatch) {
      ...
      // Add new match
      axios
        .post("http://localhost:5000/api/auth/addMatch_car", newMatch)
        .then((response) => {
  ...
  const handleRowClick = (match) => {
    ...
    axios
      .get(`http://localhost:5000/api/auth/getScores_car/${match._id}`)
      .then((response) => setScores(response.data))
      .catch((error) => console.error(error));
    axios
      .get(`http://localhost:5000/api/auth/getPlayers_car/${match._id}?team=TeamA`)
      .then((response) => setPlayersTeamA(response.data))
      .catch((error) => console.error(error));

    axios
      .get(`http://localhost:5000/api/auth/getPlayers_car/${match._id}?team=TeamB`)
  ...
  const handleAddScore = () => {
    if (selectedMatch) {
      axios
        .post(
          `http://localhost:5000/api/auth/addScores_car/${selectedMatch._id}`,{
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
            round2: parseInt(scoreFormData.round2), // Parse round2 to integer
            round3: parseInt(scoreFormData.round3), // Parse round3 to integer
          }
  ...
  const handleUpdateScore = () => {
    if (selectedMatch && selectedScoreDetails) {
      ...
      axios
        .put(
          `http://localhost:5000/api/auth/updateScoredetails_car/${selectedScoreDetails._id}`,
          {
            ...scoreFormData,
            round1: parseInt(scoreFormData.round1), // Parse round1 to integer
            round2: parseInt(scoreFormData.round2), // Parse round2 to integer
  ...
  const handleDeleteScore = () => {
    if (selectedScoreDetails) {
      ...
      axios
        .delete(
          `http://localhost:5000/api/auth/deleteScoredetails_car/${scoreId}`,
          {
  ...
  const handleUpdatePlayer = () => {
    if (selectedMatch && selectedPlayerDetails) {
      ...
      axios
        .put(`http://localhost:5000/api/auth/updatePlayerDetails_car/${_id}`, {
  ...
  const handleDeletePlayer = () => {
    if (selectedPlayerDetails) {
      axios
        .delete(
          `http://localhost:5000/api/auth/deletePlayerDetails_car/${selectedPlayerDetails._id}`
        )
        .then(() => {
          const updatedPlayers =
